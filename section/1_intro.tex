...
Large language models (LLMs), including frontier systems such as GPT-4~\cite{openai2023gpt4}, Gemini~\cite{team2023gemini}, and Claude~\cite{anthropic2024claude35sonnettool}, have evolved from text generators into general-purpose reasoning engines capable of instruction following, task decomposition, tool use, planning, and agentic interaction with external environments~\citep{brown2020language,wei2022chain}. 
A central reason for this transition is that large-scale pretraining allows LLMs to encode broad forms of \emph{world knowledge}: semantic knowledge about objects and events, commonsense knowledge about everyday situations, procedural knowledge about how tasks are performed, and causal knowledge about likely consequences of actions~\citep{petroni2019language,roberts2020knowledge,kandpal2023longtail,chang2024factualpretraining}. 
Such knowledge makes LLMs useful high-level priors for systems that must reason about, interact with, and eventually act in the physical world.

However, LLM-based world knowledge alone is insufficient for \emph{Physical AI}. 
...
Language provides an effective abstraction interface, but it is too sparse and lossy to represent physical states such as trajectories, velocities, contacts, deformations, occlusions, high-frequency dynamics, and long-horizon temporal evolution. 
Thus, the key question is not only whether LLMs contain world knowledge, but how such knowledge can be grounded into perception, prediction, simulation, planning, policy learning, and real-world action.

Vision-language models (VLMs) and multimodal large language models (MLLMs) provide the first step in this grounding process. 
By connecting language with images, videos, regions, objects, spatial relations, and affordances, they allow LLM-derived world knowledge to become situated in perceptual observations~\citep{alayrac2022flamingo,li2023blip,liu2023llava,team2023gemini}. 
For Physical AI, this is crucial: an agent must not only know that a glass is fragile or that a handle affords pulling, but must also determine whether the relevant object is present, where it is, and whether the current configuration makes the action feasible. 
Yet many VLMs still express physical understanding primarily through language outputs. 
This makes them effective for high-level description and reasoning, but limited for dense physical prediction, continuous control, and action-conditioned future modeling.

Vision-Language-Action (VLA) models move one step further, from perception and description toward executable behavior. 
By mapping visual observations and language instructions to actions, VLAs provide an action-facing interface between multimodal reasoning and embodied control~\citep{driess2023palm,zitkovich2023rt,kim2024openvla,black2024pi0}. 
Nevertheless, VLAs alone are not sufficient for general Physical AI. 
Their action representations are often embodiment-specific, their robot data are limited compared with web-scale language and vision data, and their generalization across tasks, environments, and robot bodies remains fragile. 
More importantly, although VLAs connect perception and language to action, they typically do not provide an internal predictive model of how the physical world evolves under actions.

This motivates the role of \emph{world models}. 
In contrast to LLM-based world knowledge, which captures what is semantically, procedurally, or causally plausible, world models aim to predict or simulate what will happen next under physical dynamics~\citep{ha2018world,lecun2022path}. 
They mark a transition beyond purely LLM-centric backbones: instead of only relying on language-encoded priors, world models directly learn predictive and simulative knowledge from videos, trajectories, and embodied interactions. 
Video world models provide visual imagination and future prediction; latent world models learn compact predictive representations for planning and control; and interactive or action-conditioned world models allow agents to reason about counterfactual futures before acting. 
In this sense, LLMs and world models are complementary: LLMs encode high-level knowledge about what typically happens and what actions may be meaningful, whereas world models estimate what is likely to happen next.
...
This roadmap keeps the survey centered on language and multimodal reasoning while clarifying how recent Physical AI systems connect to broader embodied intelligence. 
...
Third, we discuss VLA models as a transition from multimodal grounding to executable action, emphasizing both their promise and their limitations as generalist physical learners. 
Fourth, we survey world models as predictive and simulative substrates for Physical AI. 
Finally, we discuss the gap between current models and deployable Physical AI systems, including policy learning, sim-to-real transfer, closed-loop evaluation, safety, reproducibility, and closed-source frontier systems.
...
\begin{itemize}
    ...
    We formulate Physical AI as systems that ground LLM-based world knowledge and world-model-based predictive knowledge into perception, prediction, simulation, planning, policy learning, and real-world action.
...
% extend LLMs with visual perception, allowing language-based world knowledge to be grounded into images, videos, scenes, objects, spatial relations, and affordances~\citep{alayrac2022flamingo,liu2023llava,openai2023gpt4}. 
...
% By mapping visual observations and language instructions into executable actions, VLAs provide a natural interface between multimodal reasoning and embodied control~\citep{zitkovich2023rt,driess2023palm,kim2024openvla}. 
